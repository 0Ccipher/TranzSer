/*TranZSER test*/#include <pthread.h>
#include <stdatomic.h>
#include <assert.h>

void __VERIFIER_Transaction_begin();
void __VERIFIER_Transaction_end();
#define begin __VERIFIER_Transaction_begin()
#define end __VERIFIER_Transaction_end()
atomic_int vars[3]; 
atomic_int atom_0_r3_2; 
atomic_int atom_0_r7_1; 
atomic_int atom_1_r3_2; 
atomic_int atom_1_r6_1; 
atomic_int atom_2_r3_2; 
atomic_int atom_2_r6_1; 

atomic_int __fence_var;

void *t0(void *arg){
 begin;
label_1:;
  atomic_store_explicit(&vars[0], 2, memory_order_seq_cst);
  int v2_r3 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v3_r4 = v2_r3 ^ v2_r3;
  atomic_store_explicit(&vars[1+v3_r4], 1, memory_order_seq_cst);
  int v5_r7 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v38 = (v2_r3 == 2);
  atomic_store_explicit(&atom_0_r3_2, v38, memory_order_seq_cst);
  int v39 = (v5_r7 == 1);
  atomic_store_explicit(&atom_0_r7_1, v39, memory_order_seq_cst);
  end;
  return NULL;
}




void *t1(void *arg){
 begin;
label_2:;
  atomic_store_explicit(&vars[1], 2, memory_order_seq_cst);
  int v7_r3 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v8_r4 = v7_r3 ^ v7_r3;
  int v9_r4 = v8_r4 + 1;
  atomic_store_explicit(&vars[2], v9_r4, memory_order_seq_cst);
  int v11_r6 = atomic_load_explicit(&vars[2], memory_order_seq_cst);
  int v40 = (v7_r3 == 2);
  atomic_store_explicit(&atom_1_r3_2, v40, memory_order_seq_cst);
  int v41 = (v11_r6 == 1);
  atomic_store_explicit(&atom_1_r6_1, v41, memory_order_seq_cst);
  end;
  return NULL;
}




void *t2(void *arg){
 begin;
label_3:;
  atomic_store_explicit(&vars[2], 2, memory_order_seq_cst);
  int v13_r3 = atomic_load_explicit(&vars[2], memory_order_seq_cst);
  int v14_r4 = v13_r3 ^ v13_r3;
  int v15_r4 = v14_r4 + 1;
  atomic_store_explicit(&vars[0], v15_r4, memory_order_seq_cst);
  int v17_r6 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v42 = (v13_r3 == 2);
  atomic_store_explicit(&atom_2_r3_2, v42, memory_order_seq_cst);
  int v43 = (v17_r6 == 1);
  atomic_store_explicit(&atom_2_r6_1, v43, memory_order_seq_cst);
  end;
  return NULL;
}




int main(int argc, char **argv){
  pthread_t thr0; 
  pthread_t thr1; 
  pthread_t thr2; 

  pthread_create(&thr0, NULL , t0, NULL);
  pthread_create(&thr1, NULL , t1, NULL);
  pthread_create(&thr2, NULL , t2, NULL);

  pthread_join(thr0,NULL);
  pthread_join(thr1,NULL);
  pthread_join(thr2,NULL);

   int res = 0;
 begin;
  int v18 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v19 = (v18 == 2);
  int v20 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v21 = (v20 == 2);
  int v22 = atomic_load_explicit(&vars[2], memory_order_seq_cst);
  int v23 = (v22 == 2);
  int v24 = atomic_load_explicit(&atom_0_r3_2, memory_order_seq_cst);
  int v25 = atomic_load_explicit(&atom_0_r7_1, memory_order_seq_cst);
  int v26 = atomic_load_explicit(&atom_1_r3_2, memory_order_seq_cst);
  int v27 = atomic_load_explicit(&atom_1_r6_1, memory_order_seq_cst);
  int v28 = atomic_load_explicit(&atom_2_r3_2, memory_order_seq_cst);
  int v29 = atomic_load_explicit(&atom_2_r6_1, memory_order_seq_cst);
  int v30_conj = v28 & v29;
  int v31_conj = v27 & v30_conj;
  int v32_conj = v26 & v31_conj;
  int v33_conj = v25 & v32_conj;
  int v34_conj = v24 & v33_conj;
  int v35_conj = v23 & v34_conj;
  int v36_conj = v21 & v35_conj;
  int v37_conj = v19 & v36_conj;
  if (v37_conj == 1) res = 1;
  end;
 if(res == 1) assert(0);  return 0;
}
