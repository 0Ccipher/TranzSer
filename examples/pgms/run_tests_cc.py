...
def runall():
    ...
    for tst in tests:
        ...
        n=n+1
        tst = tst.split("\n")
        cmd = f"nidhuggc -cc -extfun-no-race=printf {tst[0]}"
        try:
        ...
        if output.find("Error: Assertion violation at ") >= 0:
            text = colored('   UNSAFE' , 'red')
            print(text)
            res = 0
            unsafe = unsafe + 1
            logfile.write(tst[0]+'  UNSAFE \n') #:---Assertion Violation \n')
        else:
            parts_0=output.split(":=")
            trace = parts_0[-1].split("\n")
            text = colored('   SAFE' , 'green')
            print(text + " ---> No of traces: " +trace[0])
            res = 1
            safe = safe + 1
            logfile.write(tst[0]+'   SAFE \n')#: ---No of traces = '+trace[0]+'\n')
        try:
