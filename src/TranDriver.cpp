...
View TranDriver::calcBasicHbView(Event e) const
{
...
View TranDriver::calcBasicPorfView(Event e) const
{
...
void TranDriver::calcBasicViews(EventLabel *lab)
{
...
void TranDriver::calcReadViews(ReadLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
	...
	if (!lab->getRf().isBottom()) {
		...
		if(!lab->getTransaction().isInvalid()){
			if(!rfLab->getTransaction().isInvalid() && rfLab->getTransaction() != lab->getTransaction() ){
				...
				auto *eLab = g.getEventLabel(endEv);
				porf.update(eLab->getPorfView());
				hb.update(eLab->getPorfView());
			}
		}
	}
...
void TranDriver::calcWriteViews(WriteLabel *lab)
{
	calcBasicViews(lab);
...
void TranDriver::calcWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void TranDriver::calcRMWWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void TranDriver::calcFenceRelRfPoBefore(Event last, View &v)
{
	const auto &g = getGraph();
...
void TranDriver::calcFenceViews(FenceLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void TranDriver::calcStartViews(ThreadStartLabel *lab)
{
	const auto &g = getGraph();
...
void TranDriver::calcJoinViews(ThreadJoinLabel *lab)
{
	const auto &g = getGraph();
...
void TranDriver::updateLabelViews(EventLabel *lab, const EventDeps *deps) /* deps ignored */
{
	const auto &g = getGraph();
...
bool TranDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
{
...
Event TranDriver::findRaceForNewLoad(const ReadLabel *rLab)
{
	const auto &g = getGraph();
...
Event TranDriver::findRaceForNewStore(const WriteLabel *wLab)
{
	const auto &g = getGraph();
...
Event TranDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
{
...
void TranDriver::changeRf(Event read, Event store)
{
	auto &g = getGraph();
...
void TranDriver::updateStart(Event create, Event start)
{
	auto &g = getGraph();
...
bool TranDriver::updateJoin(Event join, Event childLast)
{
	auto &g = getGraph();
...
void TranDriver::initConsCalculation()
{
	return;
}
