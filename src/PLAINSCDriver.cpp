...
PLAINSCDriver::PLAINSCDriver(std::shared_ptr<const Config> conf, std::unique_ptr<llvm::Module> mod,
		       std::unique_ptr<ModuleInfo> MI)
	: GenMCDriver(conf, std::move(mod), std::move(MI))
{
	auto &g = getGraph();

	/* PLAINSC requires the calculation of PSC */
...
View PLAINSCDriver::calcBasicHbView(Event e) const
{
...
View PLAINSCDriver::calcBasicPorfView(Event e) const
{
...
void PLAINSCDriver::calcBasicViews(EventLabel *lab)
{
...
void PLAINSCDriver::calcReadViews(ReadLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void PLAINSCDriver::calcWriteViews(WriteLabel *lab)
{
	calcBasicViews(lab);
...
void PLAINSCDriver::calcWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void PLAINSCDriver::calcRMWWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void PLAINSCDriver::calcFenceRelRfPoBefore(Event last, View &v)
{
	const auto &g = getGraph();
...
void PLAINSCDriver::calcFenceViews(FenceLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void PLAINSCDriver::calcStartViews(ThreadStartLabel *lab)
{
	const auto &g = getGraph();
...
void PLAINSCDriver::calcJoinViews(ThreadJoinLabel *lab)
{
	const auto &g = getGraph();
...
void PLAINSCDriver::updateLabelViews(EventLabel *lab, const EventDeps *deps) /* deps ignored */
{
	const auto &g = getGraph();
...
bool PLAINSCDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
{
...
Event PLAINSCDriver::findRaceForNewLoad(const ReadLabel *rLab)
{
	const auto &g = getGraph();
...
Event PLAINSCDriver::findRaceForNewStore(const WriteLabel *wLab)
{
	const auto &g = getGraph();
...
Event PLAINSCDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
{
...
void PLAINSCDriver::changeRf(Event read, Event store)
{
	auto &g = getGraph();
...
void PLAINSCDriver::updateStart(Event create, Event start)
{
	auto &g = getGraph();
...
bool PLAINSCDriver::updateJoin(Event join, Event childLast)
{
	auto &g = getGraph();
...
void PLAINSCDriver::initConsCalculation()
{
	return;
}
