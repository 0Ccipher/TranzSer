...
def runallnidhugg():
    ...
    version = subprocess.check_output(["./src/genmc", '--version'], stderr = subprocess.STDOUT).decode()
    logfile.write('# ' + version)
    logfile.write('# Date: ' + datetime.datetime.now().strftime('%y%m%d-%H:%M') + '\n')
    logfile.write(str([GENMC, '--mo','--rc11','--check-consistency-type=full', '--check-consistency-point=step']))
    ...
    for tst in tests:
        n = n+1
        try:
            out = subprocess.check_output([ GENMC, '--mo','--rc11','--check-consistency-type=full', '--check-consistency-point=step',
            ...
            tracecount = 0
            for line in parts_0:
                if(line.find("Number of complete executions explored:") >= 0):
                    tracecount = int(line.split(":")[-1])
                    break
            totaltracecount += tracecount
            # print([parts_0[0], tracecount])
...
def runonenidhugg(filename):
    ...
    for tst in tests:
        if tst['tstname'] != filename:
            ...
            found = True
            try:
                out = subprocess.check_output([NIDHUGGBIN, '--sc','--optimal',
                                               LITMUSDIR + '/' + tst['tstname'] + '.c'],
                                              stderr=subprocess.STDOUT).decode()
                parts_0 = out.split("\n")
                error_info = parts_0[-6]
