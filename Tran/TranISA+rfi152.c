void __VERIFIER_Transaction_begin();
void __VERIFIER_Transaction_end();

#define begin __VERIFIER_Transaction_begin()
#define end __VERIFIER_Transaction_end() 


#include <pthread.h>
#include <stdatomic.h>
#include <assert.h>

atomic_int vars[2]; 
atomic_int atom_0_r3_2; 
atomic_int atom_0_r6_1; 
atomic_int atom_1_r3_2; 
atomic_int atom_1_r6_1; 

atomic_int __fence_var;

void *t0(void *arg){
label_1:;
  begin;
  atomic_store_explicit(&vars[0], 2, memory_order_seq_cst);
  int v2_r3 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v3_r4 = v2_r3 ^ v2_r3;
  int v4_r4 = v3_r4 + 1;
  atomic_store_explicit(&vars[1], v4_r4, memory_order_seq_cst);
  int v6_r6 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v26 = (v2_r3 == 2);
  atomic_store_explicit(&atom_0_r3_2, v26, memory_order_seq_cst);
  int v27 = (v6_r6 == 1);
  atomic_store_explicit(&atom_0_r6_1, v27, memory_order_seq_cst);
    end;
return NULL;
}




void *t1(void *arg){
label_2:;
  begin;
  atomic_store_explicit(&vars[1], 2, memory_order_seq_cst);
  int v8_r3 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v9_r4 = v8_r3 ^ v8_r3;
  int v10_r4 = v9_r4 + 1;
  atomic_store_explicit(&vars[0], v10_r4, memory_order_seq_cst);
  int v12_r6 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v28 = (v8_r3 == 2);
  atomic_store_explicit(&atom_1_r3_2, v28, memory_order_seq_cst);
  int v29 = (v12_r6 == 1);
  atomic_store_explicit(&atom_1_r6_1, v29, memory_order_seq_cst);
    end;
return NULL;
}




int main(int argc, char **argv){
  pthread_t thr0; 
  pthread_t thr1; 

  pthread_create(&thr0, NULL , t0, NULL);
  pthread_create(&thr1, NULL , t1, NULL);

  pthread_join(thr0,NULL);
  pthread_join(thr1,NULL);

  int v13 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v14 = (v13 == 2);
  int v15 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v16 = (v15 == 2);
  int v17 = atomic_load_explicit(&atom_0_r3_2, memory_order_seq_cst);
  int v18 = atomic_load_explicit(&atom_0_r6_1, memory_order_seq_cst);
  int v19 = atomic_load_explicit(&atom_1_r3_2, memory_order_seq_cst);
  int v20 = atomic_load_explicit(&atom_1_r6_1, memory_order_seq_cst);
  int v21_conj = v19 & v20;
  int v22_conj = v18 & v21_conj;
  int v23_conj = v17 & v22_conj;
  int v24_conj = v16 & v23_conj;
  int v25_conj = v14 & v24_conj;
  if (v25_conj == 1) assert(0);
  return 0;
}
