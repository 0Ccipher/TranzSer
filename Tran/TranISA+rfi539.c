void __VERIFIER_Transaction_begin();
void __VERIFIER_Transaction_end();

#define begin __VERIFIER_Transaction_begin()
#define end __VERIFIER_Transaction_end() 


#include <pthread.h>
#include <stdatomic.h>
#include <assert.h>

atomic_int vars[6]; 
atomic_int atom_0_r1_1; 
atomic_int atom_0_r6_1; 
atomic_int atom_1_r1_1; 
atomic_int atom_1_r5_1; 
atomic_int atom_2_r1_1; 
atomic_int atom_2_r5_1; 

atomic_int __fence_var;

void *t0(void *arg){
label_1:;
  begin;
  int v2_r1 = atomic_load_explicit(&vars[0], memory_order_seq_cst);
  int v3_r3 = v2_r1 ^ v2_r1;
  atomic_store_explicit(&vars[1+v3_r3], 1, memory_order_seq_cst);
  int v5_r6 = atomic_load_explicit(&vars[1], memory_order_seq_cst);
  int v6_r7 = v5_r6 ^ v5_r6;
  int v7_r7 = v6_r7 + 1;
  atomic_store_explicit(&vars[2], v7_r7, memory_order_seq_cst);
  int v35 = (v2_r1 == 1);
  atomic_store_explicit(&atom_0_r1_1, v35, memory_order_seq_cst);
  int v36 = (v5_r6 == 1);
  atomic_store_explicit(&atom_0_r6_1, v36, memory_order_seq_cst);
    end;
return NULL;
}




void *t1(void *arg){
label_2:;
  begin;
  int v9_r1 = atomic_load_explicit(&vars[2], memory_order_seq_cst);
  int v10_r3 = v9_r1 ^ v9_r1;
  int v11_r3 = v10_r3 + 1;
  atomic_store_explicit(&vars[3], v11_r3, memory_order_seq_cst);
  int v13_r5 = atomic_load_explicit(&vars[3], memory_order_seq_cst);
  int v14_r6 = v13_r5 ^ v13_r5;
  int v15_r6 = v14_r6 + 1;
  atomic_store_explicit(&vars[4], v15_r6, memory_order_seq_cst);
  int v37 = (v9_r1 == 1);
  atomic_store_explicit(&atom_1_r1_1, v37, memory_order_seq_cst);
  int v38 = (v13_r5 == 1);
  atomic_store_explicit(&atom_1_r5_1, v38, memory_order_seq_cst);
    end;
return NULL;
}




void *t2(void *arg){
label_3:;
  begin;
  int v17_r1 = atomic_load_explicit(&vars[4], memory_order_seq_cst);
  int v18_r3 = v17_r1 ^ v17_r1;
  int v19_r3 = v18_r3 + 1;
  atomic_store_explicit(&vars[5], v19_r3, memory_order_seq_cst);
  int v21_r5 = atomic_load_explicit(&vars[5], memory_order_seq_cst);
  int v22_r6 = v21_r5 ^ v21_r5;
  int v23_r6 = v22_r6 + 1;
  atomic_store_explicit(&vars[0], v23_r6, memory_order_seq_cst);
  int v39 = (v17_r1 == 1);
  atomic_store_explicit(&atom_2_r1_1, v39, memory_order_seq_cst);
  int v40 = (v21_r5 == 1);
  atomic_store_explicit(&atom_2_r5_1, v40, memory_order_seq_cst);
    end;
return NULL;
}




int main(int argc, char **argv){
  pthread_t thr0; 
  pthread_t thr1; 
  pthread_t thr2; 

  pthread_create(&thr0, NULL , t0, NULL);
  pthread_create(&thr1, NULL , t1, NULL);
  pthread_create(&thr2, NULL , t2, NULL);

  pthread_join(thr0,NULL);
  pthread_join(thr1,NULL);
  pthread_join(thr2,NULL);

  int v24 = atomic_load_explicit(&atom_0_r1_1, memory_order_seq_cst);
  int v25 = atomic_load_explicit(&atom_0_r6_1, memory_order_seq_cst);
  int v26 = atomic_load_explicit(&atom_1_r1_1, memory_order_seq_cst);
  int v27 = atomic_load_explicit(&atom_1_r5_1, memory_order_seq_cst);
  int v28 = atomic_load_explicit(&atom_2_r1_1, memory_order_seq_cst);
  int v29 = atomic_load_explicit(&atom_2_r5_1, memory_order_seq_cst);
  int v30_conj = v28 & v29;
  int v31_conj = v27 & v30_conj;
  int v32_conj = v26 & v31_conj;
  int v33_conj = v25 & v32_conj;
  int v34_conj = v24 & v33_conj;
  if (v34_conj == 1) assert(0);
  return 0;
}
